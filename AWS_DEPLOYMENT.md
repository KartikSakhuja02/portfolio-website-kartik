...
For live website traffic bars, the dashboard uses the API request stream generated by the app itself, so the backend must stay behind the same public endpoint your visitors use.
...
- The network traffic panel is driven by live request history, so it reflects actual site usage rather than static demo data.
- The logs panel reads from the EC2 host log mount, so `HOST_LOG_DIR=/var/log` is the important setting on the instance.
- If the logs panel stays empty, the two most common causes are a missing host mount and using the wrong log file set for the instance OS. Restart the container after setting `HOST_LOG_DIR`, and check both `/var/log/syslog` and `/var/log/messages` on the host.
